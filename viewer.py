...
def view_script(uri, duration):
    ...
    player_args = ['python', uri]
    player_kwargs = {'_bg': True, '_ok_code': [0], '_timeout': SCRIPT_TIMEOUT + int(duration.split('.')[0])}
...
def asset_loop(scheduler):
    ...
    if asset is None:
        ...
        if 'image' in mime:
            ...
            view_script(uri, asset["duration"])
        else:
